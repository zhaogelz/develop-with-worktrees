...
`start` selects the least-recently-used idle slot and derives a task branch from the invocation worktree's current local branch. It records the branch, base commit, base worktree, and a monotonically increasing slot generation. `commit` requires an exact complete path manifest. `ready` safely synchronizes a forward base, validates the Ready closure, and records proof. Each profile rechecks the expected base and exact candidate after entering the machine validation queue and before each command; Ready checks both again after validation. If another Finish advanced the base, the same Ready call resynchronizes and reuses exact unchanged profile proofs. Five automatic retries bound convergence; continued movement preserves the task and fails explicitly instead of recording a stale Ready proof.

`finish` takes the local FIFO integration turn, freezes the exact task/slot/worktree/branch/base/candidate/proof identity in a state transaction, and only then fast-forwards the recorded base by candidate SHA. The transaction moves `prepared → promoted → completed`. `recover` publishes a task operation before it classifies an interruption from that transaction and Git ancestry, so development processes cannot race recovery. An already integrated candidate proceeds only through exact cleanup, while a normally advanced base that does not contain the candidate returns the task to Active with Ready evidence cleared. Rewritten bases, dirty or moved worktrees, moved refs, unknown ignored content, or unresolvable Git facts preserve the task and fail closed. Completion first leaves the slot in an unallocatable `release-checking` state; only an exact final worktree check publishes it idle, and Start revalidates the released directory identity plus protected ignored content before any reset or switch. The completed receipt is a rebuildable projection, not the transaction log. Strictly matching schema-1 receipts are migrated under the same lock; malformed or conflicting legacy evidence is never guessed.

`abandon` uses its own persisted transaction under the same FIFO integration lock and the slot-maintenance lock. It never operates on another branch, never discards tracked working-tree changes, and never uses blanket `git clean`. Ordinary untracked files are deleted only through an unchanged-object handle; protected, retained, unknown ignored, replaced, or late content blocks release. The task ref is deleted in one Git transaction that also verifies every other active task ref. An integrated candidate or one referenced by another active task cannot be reported as abandoned, and content detected at the final release seam quarantines the slot before it can be reused.
